...
from aospy.engine.combat import _effective_counts, _intrinsic_bonus, expected_weapon_damage
...
def line_avg_dmg_pen_weighted(unit) -> float:
    ...
    for w, c in zip(unit.weapons, counts, strict=True):
        ...
        product = dmg_save6 * pen  # == dmg_save2 de la ligne
...
FEATURES_F = tuple("dmg_pen_lineavg_w" if f == "dmg_vs_save2" else f for f in FEATURES_B)  # variante testée ici
