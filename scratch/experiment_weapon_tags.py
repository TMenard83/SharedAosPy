"""Essai : les tags de texte libre `Weapon.abilities` (Anti-<MOT-CLÉ>, Charge
(+N <Stat>), type de Crit) expliquent-ils une part du prix que le modèle de
coût officiel (`cost_model.fit_cost_model_factorial`) rate aujourd'hui ?

Contexte : un bug de parsing dans `engine/combat.py` (`_ANTI_RE`/`_CHARGE_RE`
ne toléraient pas le markup **gras**/^^exposant^^ de BSData ni le HTML
<span> de Wahapedia) empêchait la quasi-totalité des Anti-X réels de
s'appliquer même en combat — corrigé séparément (159/159 unités concernées
matchent maintenant). Même corrigé, ce bonus reste invisible aux features
officielles de `features.py` : ses défenseurs sondes (`_PROBE_SAVE2` etc.)
n'ont aucun mot-clé, et `_NO_MODS` n'a jamais `attacker_charged=True` — donc
`dmg_vs_save2`/`dmg_vs_nosave` ne peuvent physiquement pas refléter un Anti-X
ou un Charge (+N), quel que soit l'état du bug de parsing.

Ici on recalcule, pour chaque unité, le supplément de dégât (à save 2+) que
son Anti-X (contre une cible portant tous les mots-clés jamais visés dans la
base) et son Charge (+N) apporteraient — un signal aujourd'hui totalement
absent du modèle officiel — et on teste s'il explique une part significative
du prix, en plus (ou à la place) d'un simple indicateur catégoriel de
présence du tag.
...
model_a = cost_model.fit_cost_model_factorial(frame=frame.copy())  # officiel, sans tags

cats_b = list(cost_model.CATEGORICAL_FEATURES) + ["has_anti", "has_charge", "crit_type"]
model_b = cost_model.fit_cost_model_factorial(categorical=cats_b, frame=frame.copy())

feats_c = list(cost_model.MODEL_FEATURES) + ["anti_bonus_save2", "charge_bonus_save2"]
model_c = cost_model.fit_cost_model_factorial(features=feats_c, frame=frame.copy())

feats_d = list(cost_model.MODEL_FEATURES) + ["anti_bonus_save2", "charge_bonus_save2"]
cats_d = list(cost_model.CATEGORICAL_FEATURES) + ["crit_type"]
model_d = cost_model.fit_cost_model_factorial(features=feats_d, categorical=cats_d, frame=frame.copy())

print(f"Modèle A (officiel, sans tags)                      : R² = {model_a.r_squared:.4f}  R² ajusté = {model_a.adj_r_squared:.4f}")
...
print(f"Modèle D (magnitude continue + crit_type catégoriel)   : R² = {model_d.r_squared:.4f}  R² ajusté = {model_d.adj_r_squared:.4f}  (gain {model_d.adj_r_squared - model_a.adj_r_squared:+.4f})")
print()
