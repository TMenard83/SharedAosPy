...
(scratch/per_unit_nocharge_TM.json) et le registre de coût (plan d'expérience
...
def render(joined: list[dict], out_path: str, *, faction: str | None = None, universe_n: int = 0) -> None:
    ...
    story.append(Paragraph(
        "<b>Méthode :</b> ce registre croise le <i>Compendium des Duels — Unités</i> (performance en "
        ...
        "Anomalies de Coût — Unités</i> (résidu d'un plan d'expérience factoriel ajusté séparément sur les "
        "troupes) — les deux sur les mêmes unités, jointes par identifiant interne. Les unités dont la "
        f"prédiction de coût est trop faible (&lt; {MIN_PREDICTED:.0f} pts, résidu relatif peu fiable) sont "
        "écartées, comme dans le registre des coûts. Voir <i>Panthéon des Champions — Héros</i> pour le "
        "pendant héros, dont le plan de coût est ajusté indépendamment.<br/><br/>"
        "<b>Pourquoi croiser :</b> un bon ROI seul peut simplement signaler une unité déjà chère et donc "
        "« normale » côté prix. Une forte sous-cotation seule ne dit rien de l'efficacité réelle en "
        "combat. Une unité vraiment excellente cumule les deux : elle gagne ses duels <i>et</i> coûte moins "
        ...
        "profite donc au score). Les z-scores sont calculés sur la population des unités de troupe communes "
        "aux deux registres, chaque quantité étant centrée-réduite (moyenne 0, écart-type 1) pour la rendre "
        "comparable malgré des échelles différentes (pourcentage de ROI vs pourcentage de résidu). Un score "
        "de +1 signifie une unité un écart-type au-dessus de la moyenne sur ce critère combiné — pas une "
        "grandeur physique en soi, un simple outil de tri.<br/><br/>"
        + (
